...
// Implement in SoftataDevice_OLED096::Misc(int cmd
...
bool SoftataDevice_OLED096::Setup(byte * settings, byte numSettings)
{
  if(numSettings>0)
  {
    int i2c = settings[0];
...
Tristate SoftataDevice_OLED096::Dummy()
{
  return notImplemented;
}


Tristate SoftataDevice_OLED096::Clear()
{
  u8g2.clearBuffer();
...
Tristate SoftataDevice_OLED096::CursorWriteStringAvailable()
{
	return (Tristate)true;
}

Tristate SoftataDevice_OLED096::SetCursor(byte col, byte line)
{
...
Tristate SoftataDevice_OLED096::WriteString(String msg)
{
    //u8g2.setFont(u8g2_font_ncenB08_tr);   // choose a suitable font
...
Tristate SoftataDevice_OLED096::WriteString (byte col, byte line, String msg)
{
  int y = LINEHEIGHT*line + FIRSTLINESTART;
  int x = COLWIDTH*col;
...
Tristate SoftataDevice_OLED096::Home ()
{
  u8g2.home(); 
  return (Tristate)true;
}
...
Tristate SoftataDevice_OLED096::Misc(byte cmd, byte * data, byte length)
{
  if (cmd<0)
...
// Not relevant for SoftataDevice_OLED096
Tristate SoftataDevice_OLED096::Backlight()
{
  return notImplemented;;
}
