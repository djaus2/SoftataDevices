...
Methods that do not return a value (such as with a sensor read), return a triste value which is normally ```ok```. If there is an error in the method, ````_nok``` is returned. If a method is not implemneted then ```notimplemneted``` is returned which at a higher level is meant to be interpretteed is OK but can be used otherwise. ```_nan_``` is alos a return option for data reads.
