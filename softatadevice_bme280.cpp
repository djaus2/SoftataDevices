...
SoftataDevice_BME280::SoftataDevice_BME280()
{
    SoftataDevice_Sensor::num_properties = NUM_BME280_PROPERTIES;
    SoftataDevice_Sensor::sensorType = BME280;
}
...
bool SoftataDevice_BME280::Setup(byte * settings, byte numSettings)
{
    if(numSettings>0)
    {
    int i2c = settings[0];
...
bool SoftataDevice_BME280::Setup()
{
    SoftataDevice::SetI2CPins(0);
    return SetupBME280();
}

bool SoftataDevice_BME280::SetupBME280()
{
    while (!Serial);
...
String SoftataDevice_BME280::GetPins()
{
    String msg="BME280 Sensor Pins:";
    msg.concat(BME280_PINOUT);
    return msg;       
}
...
String SoftataDevice_BME280::GetTelemetry()
{
  double values[3];
  if (ReadAll(values))
  {
...
CallbackInfo * SoftataDevice_BME280::GetCallbackInfo()
{
    info.period = 314159;
    info.next = 137;
...
double SoftataDevice_BME280::Read(int property)
{
